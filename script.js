...
function initParticles(id){
  ...
  const cols=['rgba(139,94,60,','rgba(232,220,200,','rgba(107,66,38,','rgba(212,197,169,'];
  const mk=()=>({x:Math.random()*W,y:Math.random()*H,r:Math.random()*1.1+.2,vy:-(Math.random()*.15+.04),vx:(Math.random()-.5)*.07,a:Math.random()*Math.PI*2,s:Math.random()*.012,o:Math.random()*.3+.03,col:cols[Math.floor(Math.random()*cols.length)]});
  const pts=Array.from({length:50},mk);
  (function draw(){ctx.clearRect(0,0,W,H);pts.forEach(p=>{p.a+=p.s;p.x+=p.vx+Math.sin(p.a)*.07;p.y+=p.vy;if(p.y<-4)Object.assign(p,mk(),{y:H+4});ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle=p.col+p.o+')';ctx.fill();});requestAnimationFrame(draw);})();
}
